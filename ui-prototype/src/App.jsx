...
const kindThemes = {
  ...
  グループ: {
    key: 'group',
    label: 'Group',
    description: '成果物の種類や領域でまとめる任意階層',
    icon: FolderKanban,
    color: '#0d9488',
    bg: '#ccfbf1'
  },
    ...
    <div className={sidebarCollapsed ? 'app-shell sidebar-collapsed' : 'app-shell'}>
      <aside className="sidebar">
        <div className="brand">
          ...
          <button
            aria-label={sidebarCollapsed ? 'サイドメニューを開く' : 'サイドメニューを閉じる'}
            className="sidebar-toggle"
            onClick={() => setSidebarCollapsed((current) => !current)}
            title={sidebarCollapsed ? 'サイドメニューを開く' : 'サイドメニューを閉じる'}
            type="button"
          >
        ...
        <nav className="nav-list">
          {menu.map((item) => {
            ...
            return (
              <button className={active === item.id ? 'nav-item active' : 'nav-item'} key={item.id} onClick={() => moveMenu(item.id)} title={item.label} type="button">
                <Icon size={18} />
      ...
      <main className="workspace">
        <header className="topbar">
          <div>
            <p className="eyebrow">WorkHub prototype</p>
...
function getReservedEventDetail(item) {
  ...
  return details[item.taskId] || {
    calendar: 'WorkHub',
    meetingUrl: 'https://meet.google.com/workhub',
    location: 'Google Meet',
...
function Tasks({ onHelp }) {
  ...
  const typeFilterOptions = useMemo(() => [
    ...
    { id: '管理区分', label: 'Management' },
    { id: 'Project', label: 'Project' },
    { id: 'マイルストーン', label: 'Milestone' },
    { id: 'カテゴリ', label: 'Category' },
    { id: 'グループ', label: 'Group' },
    { id: '成果物チケット', label: 'Deliverable' },
    { id: 'タスクチケット', label: 'Task' }
  ], []);
  ...
  const [createMenuNodeId, setCreateMenuNodeId] = useState(null);
  ...
  function startInlineDraft(parentNode, childKind = getAllowedChildKinds(parentNode)[0]) {
        ...
        <div className="wbs-table" role="table" aria-label="WBS task table">
          <div className="wbs-head" role="row" style={{ gridTemplateColumns: tableTemplate }}>
            ...
            <span className="resizable-head">
              ...
              <button aria-label="Type列を調整" className="column-resizer" onPointerDown={(event) => beginColumnResize(event, 'kind', 80)} type="button" />
            </span>
            <span className="resizable-head">
              <span className="wbs-head-title">
                <span>ステータス/進捗</span>
            ...
            <span className="resizable-head">
              <span className="wbs-head-title"><span>アサイン</span></span>
              <button aria-label="アサイン列を調整" className="column-resizer" onPointerDown={(event) => beginColumnResize(event, 'assigned', 80)} type="button" />
            </span>
            <span className="resizable-head">
          ...
          {rows.map(({ node, depth, code }) => {
            ...
            const childKinds = getAllowedChildKinds(node);
            return (
              <div
              ...
              >
                ...
                <div className="wbs-name" style={{ '--depth': depth }}>
                  ...
                  >
                    ...
                    {!node.isDraft && childKinds.length && (!hasChildren || expandedIds.has(node.id)) && hoveredNodeId === node.id ? (
                      <div className="inline-create-menu">
                        <button
                          className="inline-create-trigger"
                          onClick={(event) => {
                            event.stopPropagation();
                            setCreateMenuNodeId((current) => (current === node.id ? null : node.id));
                          }}
                          title="この下に子チケットを追加"
                          type="button"
                        >
                        ...
                        {createMenuNodeId === node.id ? (
                          <div className="inline-create-options">
                            {childKinds.map((kind) => {
                              const childTheme = kindThemes[kind] || kindThemes.default;
                ...
                <div className="assignee-cell">{node.isDraft ? '-' : getAssignedMemberName(node)}</div>
...
function getResponsibleMemberId(node) {
  ...
  return members.find((member) => member.name === node?.owner)?.id || '';
}
...
function getAssignedMemberId(node) {
  ...
  const responsibleId = getResponsibleMemberId(node);
  if (!responsibleId) return '';
  const normalized = normalizeStatus(node?.status);
  if (normalized === 'review') return members.find((member) => member.id !== responsibleId)?.id || responsibleId;
  return responsibleId;
}

function getAssignedMemberName(node) {
  const members = getMockMembers();
  return members.find((member) => member.id === getAssignedMemberId(node))?.name || '-';
}
...
function NodeDetail({ node, tree, onDrill, onSelect }) {
  ...
  const isTaskTicket = node?.kind === 'タスクチケット';
    ...
    <>
      <div className="detail-head">
        ...
        {isTaskTicket ? (
          ...
          <span className="status-tag neutral">{taskProgress ? `進捗 ${taskProgress.percent}%` : '進捗 -'}</span>
        )}
      </div>
                      ...
                      <dd className="detail-progress-inline">
                        {taskProgress ? (
                          <>
                            ...
                            <small>{taskProgress.done}/{taskProgress.total} tasks done。配下タスクがすべてdoneになると、このチケットもdone相当になります。</small>
                          </>
                        ) : (
                          <span>-</span>
                        )}
...
function getDetailNarrative(node) {
  ...
  if (node.kind === 'グループ') return '成果物を種類や領域ごとにまとめる任意階層です。グループは0個でも複数階層でもよく、配下の成果物とタスクの進捗を集計します。';
...
function getAllowedChildKinds(parentNode) {
  if (!parentNode || parentNode.kind === 'タスクチケット') return [];
  if (parentNode.kind === 'Projects' || parentNode.type === 'All') return ['Project'];
  if (parentNode.kind === 'Project') return ['マイルストーン'];
  if (parentNode.kind === '管理区分') return ['カテゴリ'];
  if (parentNode.kind === 'マイルストーン' || parentNode.kind === 'カテゴリ' || parentNode.kind === 'グループ') return ['グループ', '成果物チケット'];
