    ...
    children: [
      {
        ...
        name: 'WorkHub MVP',
        owner: '山田太郎',
        repository: 'github.com/example/workhub',
        plannedHours: 168,
        completedHours: 82,
        children: [
          {
            ...
            children: [
              {
                ...
                completedHours: 14,
                children: [
                  { id: 'task-request-form', kind: 'タスクチケット', type: 'Project', name: '業務ヒアリング登録フォーム', status: '進行中', priority: 'high', assigneeId: 'sato', estimate: 10, actual: 5, issue: '#44', pr: '-', slack: '#portfolio/102' },
                  { id: 'task-routing-rule', kind: 'タスクチケット', type: 'Project', name: 'AI化候補の担当者ルーティング', status: '未着手', priority: 'medium', assigneeId: 'tanaka', estimate: 8, actual: 0, issue: '#47', pr: '-', slack: '#portfolio/108' },
                  { id: 'task-notification-flow', kind: 'タスクチケット', type: 'Project', name: 'Slack通知フロー自動化', status: 'レビュー中', priority: 'medium', assigneeId: 'tanaka', estimate: 6, actual: 6, issue: '#49', pr: '#52', slack: '#portfolio/111' }
                ]
              },
              {
                ...
                children: [
                  ...
                  { id: 'task-file-permission-note', kind: 'タスクチケット', type: 'Project', name: 'AI参照用の共有権限メモ', status: '未着手', priority: 'low', assigneeId: 'sato', estimate: 5, actual: 0, issue: '#61', pr: '-', slack: '#portfolio/121' }
                ]
              }
            ]
          },
            ...
            children: [
              {
                ...
                completedHours: 6,
                children: [
                  { id: 'task-report-filter', kind: 'タスクチケット', type: 'Project', name: '業務領域別フィルタ', status: '進行中', priority: 'medium', assigneeId: 'suzuki', estimate: 8, actual: 3, issue: '#70', pr: '-', slack: '#portfolio/133' },
                  { id: 'task-report-export', kind: 'タスクチケット', type: 'Project', name: 'AI化候補の週次レポート出力', status: '未着手', priority: 'low', assigneeId: 'yamada', estimate: 10, actual: 0, issue: '#72', pr: '-', slack: '#portfolio/136' }
                ]
              }
            ]
          }
        ...
        children: [
          {
            ...
            children: [
              { id: 'task-sales-check', kind: 'タスクチケット', type: 'Ad-hoc', name: '営業会議ログの論点抽出', status: '完了', priority: 'medium', assigneeId: 'sato', estimate: 6, actual: 7, drive: 'drive.google.com/file/sales-check', slack: '#adhoc/443' },
              { id: 'task-lp-summary', kind: 'タスクチケット', type: 'Ad-hoc', name: '自動要約プロンプト作成', status: '進行中', priority: 'high', assigneeId: 'yamada', estimate: 8, actual: 3, drive: 'drive.google.com/file/lp-summary', slack: '#adhoc/448' }
            ]
          },
          {
            ...
            children: [
              { id: 'task-board-risk-note', kind: 'タスクチケット', type: 'Ad-hoc', name: '稟議資料の不足項目抽出', status: '進行中', priority: 'critical', assigneeId: 'yamada', estimate: 5, actual: 2, drive: 'drive.google.com/file/risk-note', slack: '#adhoc/462' }
            ]
          }
        ]
      },
        ...
        children: [
          {
            ...
            children: [
              { id: 'task-inquiry-report', kind: 'タスクチケット', type: 'Ad-hoc', name: '問い合わせログの分類軸整理', status: '進行中', priority: 'medium', assigneeId: 'sato', estimate: 5, actual: 2, drive: 'drive.google.com/file/report-fix', slack: '#adhoc/456' },
              { id: 'task-inquiry-confirmation', kind: 'タスクチケット', type: 'Ad-hoc', name: '依頼者への確認テンプレート作成', status: '未着手', priority: 'low', assigneeId: 'sato', estimate: 2, actual: 0, drive: 'drive.google.com/file/report-fix', slack: '#adhoc/457' }
            ]
          }
        ]
      }
...
export const workTree = assignWorkTreeMetadata(addDemoGroups(baseWorkTree));
...
export const resourceItems = [
  ...
  { id: 'r3', memberId: 'yamada', taskId: 'task-lp-summary', date: '2026-06-01', start: 13, duration: 1.5, type: 'Ad-hoc', label: '自動要約プロンプト作成', fixed: false },
  ...
  { id: 'r6', memberId: 'sato', taskId: 'task-inquiry-report', date: '2026-06-01', start: 10.5, duration: 2.5, type: 'Ad-hoc', label: '問い合わせログの分類軸整理', fixed: false, scheduled: true },
  ...
  { id: 'r14', memberId: 'sato', taskId: 'task-request-form', date: '2026-06-02', start: 10, duration: 2, type: 'Project', label: '業務ヒアリング登録フォーム', fixed: false, scheduled: true },
  { id: 'r15', memberId: 'tanaka', taskId: 'task-routing-rule', date: '2026-06-02', start: 13, duration: 2, type: 'Project', label: 'AI化候補の担当者ルーティング', fixed: false },
...
function addDemoGroups(nodes) {
  return nodes.map((node) => {
    const next = { ...node, children: node.children ? addDemoGroups(node.children) : undefined };
    if (next.kind !== 'マイルストーン' && next.kind !== 'カテゴリ' && next.kind !== 'グループ') return next;
    const children = next.children || [];
    const deliverables = children.filter((child) => child.kind === '成果物チケット');
    if (!deliverables.length) return next;
    const otherChildren = children.filter((child) => child.kind !== '成果物チケット');
    return {
      ...next,
      children: [
        ...otherChildren,
        {
          id: `${next.id}-group-main`,
          kind: 'グループ',
          type: next.type,
          name: getDemoGroupName(next),
...
function getDemoGroupName(node) {
  if (node.type === 'Project' && node.name.includes('リソース')) return '画面・指標グループ';
  if (node.type === 'Project' && node.name.includes('タスク')) return 'WBS運用グループ';
  if (node.type === 'Project') return 'AI化成果物グループ';
  if (node.type === 'Ad-hoc') return '自動化候補グループ';
  if (node.type === 'Explore') return '検証成果物グループ';
  return '成果物グループ';
}
